...
![image.png](https://p3-juejin-sign.byteimg.com/tos-cn-i-k3u1fbpfcp/d1b12b9dfcc54363b8389cb54e05582d~tplv-k3u1fbpfcp-jj-mark-v1:0:0:0:0:5o6Y6YeR5oqA5pyv56S-5Yy6IEAgRGFycmVu:q75.awebp?rk3s=f64ab15b&x-expires=1779247816&x-signature=3mGJr2UD3Ii8vRceupJxRf7gvfM%3D)
...
|  2  | 查看当前路径下的文件或文件夹 | 输入`dir`                              |
|  3  | 进入下一个文件夹       | 输入`cd 文件夹名称`                         |
|  4  | 进入多层级文件夹       | 输入`cd 文件夹名称\文件夹名称\.....`             |
|  5  | 返回上一层          | 输入`cd ..`                            |
|  6  | 返回当前盘符根目录      | 输入`cd \`                             |
|  7  | 清除命令行窗口界面      | 输入`cls`                              |
|  8  | 退出命令行窗口        | 输入`exit`                             |
...
jvm（java virtual machine 的缩写）是运行java代码的虚拟计算机。

可根据不同操作系统来安装不同的 jvm（不同版本的 jvm 会将java 代码编译成对应操作系统可识别的java代码，以达到编写一次 java 代码，就可以多端运行的目的，这也是所谓跨平台。

![image.png](https://p3-juejin-sign.byteimg.com/tos-cn-i-k3u1fbpfcp/b0277f7019b04ff9b3f52195e8776e91~tplv-k3u1fbpfcp-jj-mark-v1:0:0:0:0:5o6Y6YeR5oqA5pyv56S-5Yy6IEAgRGFycmVu:q75.awebp?rk3s=f64ab15b&x-expires=1779247816&x-signature=IjinusjZ8bGiAdxhT1F0%2BeeVHmY%3D)

## 2.2 jdk和jre

jdk（java development kit）全称为 java开发工具包，包含了 jre，通过 jdk 可以运行以下命令：
...
jre（java runtime environment）全称 java运行环境，包含 jvm 以及后面用到的核心类库。

jdk、jre以及jvm的关系：
jdk包含jre，而jre又包含了jvm。所以，如果安装java，只需安装jdk即可。

**注意：**
但从 jdk9 开始，jdk 目录中就没有包含 jre目录了，因为 jre作为一个运行时，里面不需要包含太多内容，内容多意味着占用空间多，同时会降低运行效率。
...
这里下载以 Macbook M1 Pro 为例子（M系列就是ADM芯片，此外就是intel系列了）。

* 先到[oracle官网](https://www.oracle.com/)；
...
**注意：** window系统安装的时候，安装路径上不要有中文和空格，下载的版本为：Windows x64 Installer
...
* 通过 `JAVA_HOME` 变量的形式来引用路径，比如：创建一个新的环境变量，让其指向 Java 的根目录。然后在 `path` 中引用这个变量，引用变量的时候要用 `%` 包裹变量，比如：引用`JAVA_HOME`，就需要使用 `%JAVA_HOME%` 这种方式才能引用，然后 `%JAVA_HOME%` 后面可以跟着 `\bin` ，这是表示会到 `JAVA_HOME` 目录下的 `bin` 目录下去查找 `java` 和 `javac` 的运行exe

**注意：** jdk安装之后，会自动在环境变量中添加javaPath环境变量，当我们配置了JAVA\_HOME之后，javaPath就可以删除了。
...
通过 `javac` 命令将 `.java` 文件编译生成一个 `.class` 文件（字节码文件）。（因为jvm只识 `.class` 文件）
...
![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/0ac6e2aea47a42b488fb646265a11008~tplv-k3u1fbpfcp-watermark.image#?w=1687\&h=1020\&s=126504\&e=png\&b=ffffff)
...
![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2276606f75b442ffa0b948dcb9dbc4ef~tplv-k3u1fbpfcp-watermark.image#?w=392\&h=90\&s=4587\&e=png\&b=fdfcfc)

<table align="center" border="1">
    <thead align="center">
        <tr>
            <th colspan="4" align="center">数据类型</th>
        </tr>
        <tr>
            <th align="center">数据类型</th>
            <th align="center">Java 常见关键字</th>
            <th align="center">Java 常见关键字</th>
            <th align="center">Java 常见关键字</th>
        </tr>
    </thead>
